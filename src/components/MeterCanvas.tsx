...
export function MeterCanvas({ metrics, settings }: MeterCanvasProps) {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      for (let ring = 0; ring < 4; ring += 1) {
        ...
        for (let index = 0; index <= 160; index += 1) {
          ...
          const harmonic = Math.sin(
            angle * (3 + ring) + frame * 0.018 * (ring + 1),
          );
          const clarity = metrics.clarity || 0.12;
          const wave =
            radius +
            harmonic * minSide * 0.014 * (1 + level * 2) +
            clarity * minSide * 0.012;
        ...
        context.closePath();
        context.strokeStyle =
          ring % 2 === 0 ? `${preset.accent}cc` : "#f6f1e955";
      ...
      for (let index = 0; index < bars; index += 1) {
        ...
        const barLevel =
          0.28 + level * 0.7 + Math.sin(index * 0.7 + frame * 0.045) * 0.16;
      ...
      context.fillText(
        metrics.pitchHz ? `${Math.round(metrics.pitchHz)} Hz` : metrics.message,
        centerX,
        centerY + minSide * 0.065,
      );
  ...
  return (
    <canvas
      ...
      aria-label="Live sound visualization"
    />
  );
}
