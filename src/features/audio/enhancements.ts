...
export async function probeEnhancements(
  onUpdate: EnhancementUpdate,
): Promise<void> {
...
async function probeOnnx(onUpdate: EnhancementUpdate): Promise<void> {
  ...
  try {
    ...
    onUpdate({
      ...
      status: "fallback",
      detail:
        error instanceof Error ? error.message : "WebAudio fallback active",
    });
  }
}
