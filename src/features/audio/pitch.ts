...
export function estimatePitch(buffer: Float32Array<ArrayBufferLike>, sampleRate: number): PitchEstimate {
  ...
  const maxLag = Math.min(buffer.length - 2, Math.ceil(sampleRate / MIN_FREQUENCY))
  let bestLag = -1
