import { probeEnhancements, INITIAL_ENHANCEMENTS, type EnhancementState } from './enhancements'
import { createImpossibleImpulse } from './impulse'
import { estimatePitch } from './pitch'
import { DEFAULT_SETTINGS, getPreset, type EngineSettings, type Preset } from './presets'
...
export class WorldVoiceEngine {
  ...
  async start(settings: EngineSettings): Promise<void> {
    ...
    if (!navigator.mediaDevices?.getUserMedia) {
      this.emit({ status: 'error', message: 'Microphone API unavailable in this browser' })
      return
    }
    ...
    try {
      ...
      this.graph = this.createGraph(this.context, this.stream, getPreset(settings.presetId))
      ...
      this.emit({
        status: 'error',
        message: error instanceof Error ? error.message : 'Could not start audio engine',
      })
      await this.stop()
    }
  }
  ...
  private createGraph(context: AudioContext, stream: MediaStream, preset: Preset): AudioGraph {
  ...
  private applySettings(settings: EngineSettings): void {
    ...
    const space = settings.batterySaver ? Math.min(settings.space, 0.36) : settings.space

    this.graph.dryGain.gain.setTargetAtTime(preset.dryGain * (1 - settings.intensity * 0.42), now, 0.05)
    this.graph.synthGain.gain.setTargetAtTime(preset.synthGain * saver, now, 0.05)
    this.graph.reverbGain.gain.setTargetAtTime(preset.reverbGain * space * saver, now, 0.08)
    this.graph.delay.delayTime.setTargetAtTime(preset.delayTime * (0.5 + space), now, 0.08)
    ...
    this.graph.convolver.buffer = createImpossibleImpulse(this.context, 1.2 + space * 3.4, settings.color)
  ...
  private startTicker(): void {
    window.clearInterval(this.tickId)
    this.tickId = window.setInterval(() => this.tick(), this.settings.batterySaver ? 66 : 33)
  }
  ...
  private tick(): void {
    ...
    this.smoothedPitch = voiced ? this.smoothedPitch * 0.78 + pitch * 0.22 : this.smoothedPitch

    for (const voice of this.graph.voices) {
      const targetPitch = Math.min(5200, Math.max(28, this.smoothedPitch * voice.ratio))
      const gain = voiced
        ? level * voice.baseGain * this.settings.intensity * (this.settings.batterySaver ? 0.72 : 1)
        : 0
  ...
  private updateEnhancement(state: EnhancementState): void {
    const enhancements = this.metrics.enhancements.map((item) => (item.id === state.id ? state : item))
