...
Vite will build the app into `docs/` with `base: "/worldvoice/"`. Built assets use hashed filenames. A copied `404.html` fallback will support any future client-side routing. The Pages clean step removes generated assets before each build while preserving authored docs such as ADRs. The `.gitignore` will ignore generic `dist/` output but must not ignore `docs/`.
